            ...
            with self._lock:
                if old_value:
                    ...
                    try:
                        ...
                        logger.info(f"Exception during updating the cache: {redis_err}")
                        retry_count += 1
                else:
                    values = [value]
                    try:
                        ...
                        logger.info(f"Exception during creating the cache: {redis_err}")
                        retry_count += 1

        if retry_count == constants.REDIS_MAX_RETRY:
            raise RedisConnectionError("Updating cache failed after max retries")
